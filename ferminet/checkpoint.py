...
def restore(restore_filename: str, batch_size: Optional[int] = None):
  ...
  with open(restore_filename, 'rb') as f:
    ...
    if ckpt_data['density_state']:
      ...
      density_state = None
    size = data.positions.shape
    if size[0] != jax.device_count():
      if (batch_size and size[0] * size[1] != batch_size):
        ...
        data = jax.tree_util.tree_map(lambda x: x.reshape((jax.device_count(), batch_size//jax.device_count())+x.shape[2:]), data)
        params = jax.tree_util.tree_map(lambda x: jax.device_get(x)[0], params)
        params = kfac_jax.utils.replicate_all_local_devices(params)
