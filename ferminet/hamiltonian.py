  ...
  def _e_l(
    ...
    if states:
      ...
      if state_specific==1:
        ...
        total_energy = []
        for i in range(states-states_update, states): # 只计算最后n个激发态的能量
          data_ = networks.FermiNetData(
          ...
          ke = local_kinetic_energy(f[i],
                                    use_scan=use_scan,
                                    complex_output=complex_output,
                                    laplacian_method=laplacian_method)
          ae, _, r_ae, r_ee = networks.construct_input_features(
              data_.positions, data_.atoms
          )
          potential = (potential_energy(r_ae, r_ee, data_.atoms, effective_charges) +
                      pp_local(r_ae) +
                      pp_nonlocal(key, f[i], params['state_{}'.format(i)], data_, ae, r_ae))
          kinetic = ke(params['state_{}'.format(i)], data_)
          total_energy.append(potential + kinetic)
        total_energy = jnp.concatenate(total_energy)
        energy_mat = None  # Not necessary for ground state
    else:
      ke = local_kinetic_energy(f,
                                use_scan=use_scan,
