...
class SpinAuxData():
  ...
  """
  estimator: networks.FermiNetData
  swap_index: int
  penalty_estimator: networks.FermiNetData
  penalty_local: networks.FermiNetData
  outlier_mask: networks.FermiNetData
  variance: networks.FermiNetData

def make_spin_penalty(
  signed_network: networks.LogFermiNetLike,
  nspins: Tuple,
  ...
  def _product(tree_leaf: networks.FermiNetData, mat: networks.FermiNetData) -> networks.FermiNetData:
  ...
  network = lambda *args, **kwargs: signed_network(*args, **kwargs)[1][0]
  ...
  #   where sz = abs(n_up - n_down) / 2 and N_\minor = min(n_up, n_down).
    ...
    def _square(params, positions, spins, atoms, changes, idx):
      return spin_calculator.full_permute_wavefn(params, positions, spins, atoms, changes)

    def _plus(params, positions, spins, atoms, changes, idx):
      _val = spin_calculator.side_permute_wavefn(params, positions, spins, atoms, changes, idx)
      return _val * minor_num

    def _plus_fix(params, positions, spins, atoms, changes, idx):
      _idx = spin_calculator.idx_in_spin[spin_calculator.sz_polar - 1][0]
      _val = spin_calculator.side_permute_wavefn(params, positions, spins, atoms, changes, _idx)
      return _val * minor_num
  ...
  spin_penalty_grad_vmapped = jax.vmap(
    jax.grad(spin_penalty, argnums=0), in_axes=(None, 0, 0, 0, 0, None))

  def _spin_penalty_grad_body(params_and_idx, data):
    params, idx = params_and_idx
    positions, spins, atoms, changes = data
    return params_and_idx, spin_penalty_grad_vmapped(params, positions, spins, atoms, changes, idx)

  fn_wavefn_grad_vmapped = jax.vmap(
    jax.grad(network, argnums=0), in_axes=(None, 0, 0, 0, 0))

  def local_value_mask(
    local_value: networks.FermiNetData,
    rm_outlier: bool = False,
    schema: str = 'deviation',
    outlier_width: float = 0.,
    outlier_bound: Tuple[float] = (1., 99.),
  ) -> Tuple[networks.FermiNetData, networks.FermiNetData]:
    ...
    if rm_outlier:
      if schema == 'deviation' and outlier_width > 0.:
        val_mean = constants.pmean(jnp.mean(local_value))
        dev_mean = constants.pmean(jnp.mean(jnp.abs(local_value - val_mean)))
        ...
        val_all_hosts = gather(local_value)
  ...
  def self_adjoint_estimator_grad(
      local_value: networks.FermiNetData,
      wf_logdet_grad: networks.ParamTree,
      mask: networks.FermiNetData,
  ) -> networks.ParamTree:
    ...
    val_mean = constants.pmean_with_mask(local_value, mask)
    ...
    estimator_grad = constants.pmean_with_structure_mask(estimator_grad, mask_tree)

    return estimator_grad

  batch_spin_penalty = jax.vmap(spin_penalty, in_axes=(None, 0, 0, 0, 0, None))
  ...
  def fn_spin_penalty(
    ...
    spin_penalty_local = batch_spin_penalty(params, data.positions, data.spins, data.atoms, data.charges, idx)
    ...
    spin_penalty_estimator = constants.pmean_with_mask(spin_penalty_local, spin_penalty_mask)
  ...
  def fn_loss_with_spin_penalty(
    ...
    key: chex.PRNGKey,
    data: networks.FermiNetData,
  ) -> Tuple[networks.FermiNetData, Tuple[SpinFuncState, SpinAuxData]]:

    sz, spin_aux = fn_spin_penalty(params, data)
  ...
  def fn_spin_grad(params, data, aux_data, wf_logdet_grad):
    ...
    _, sp_local_grad = jax.lax.scan(
      ...
      (data.positions.reshape((el_partition_num, -1, *data.positions.shape[1:])),
      data.spins.reshape((el_partition_num, -1, *data.spins.shape[1:])),
      data.atoms.reshape((el_partition_num, -1, *data.atoms.shape[1:])),
      data.charges.reshape((el_partition_num, -1, *data.charges.shape[1:]))),
      )
    ...
    sp_estimator_grad_part2 = constants.pmean_with_structure_mask(
  ...
  def spin_penalty_bwd(res, g):
    ...
    aux_data, params, data = res
    wf_logdet_grad = fn_wavefn_grad_vmapped(params, data.positions, data.spins, data.atoms, data.charges)
  ...
  def fn_fwd(
    ...
    loss, aux_data = fn_loss_with_spin_penalty(
      params, key, data
    )
    ...
    return (loss, aux_data), (aux_data, params, data)
...
class Spin:
    ...
    def __init__(self, signed_network, nspins):
        ...
        self.signed_network_vmapped = jax.vmap(signed_network, in_axes=(None, 0, 0, None, None))
        self.nspins = nspins
    ...
    def side_permute_spin(self, data: networks.FermiNetData, idx: int) -> networks.FermiNetData:
        ...
        def swap(data: networks.FermiNetData, idx1: int, idx2: int) -> networks.FermiNetData:
    ...
    def side_permute_coord(self, data: networks.FermiNetData, idx: int) -> networks.FermiNetData:
        """Swap the coordinations of a single electron in the minority spin
        state with the coordinates of all electrons in the majority spin state.
        If there are equal numbers of spin-up and -down electrons, then swap
        one single spin-up electron with all spin-down electrons.
    ...
    def side_permute_wavefn(self, params, positions, spins, atoms, charges, idx) -> networks.FermiNetData:
        ...
        """
        orig_sign, orig_logdet = self.network(params,
                                              positions,
                                              spins,
                                              atoms,
                                              charges)
        ...
        # with opposite spin.
        positions_permutation = self.side_permute_coord(positions, idx)
        spins_permutation = self.side_permute_spin(spins, idx)
        sign_permutation, logdet_permutation = self.signed_network_vmapped(
            params, positions_permutation, spins_permutation, atoms, charges)

        # calculate the wavefunction value for each permutation and sum over them.
    ...
    def full_permute_wavefn(self, params, data) -> networks.FermiNetData:
        """Sum over the wavefunction values of data in different permutations.
        ...
        side_batch_permute_wavefn = jax.vmap(
            self.side_permute_wavefn, in_axes=(None, 0, 0, 0, 0, None))
        val_permutation = side_batch_permute_wavefn(
            params, data.positions, data.spins, data.atoms, data.charges, self.idx_in_spin[self.sz_polar - 1])
        return jnp.sum(val_permutation)

    def spin_square(self, params, data) -> networks.FermiNetData:
