...
def clip_local_energys(x):
  a = jnp.nanpercentile(x, jnp.array([5, 95]), axis=0)
  return jnp.clip(x, a[0], a[1])
...
def train(cfg: ml_collections.ConfigDict, writer_manager=None):
  ...
  if data is None:
    init_data = True
    logging.info('old data is not, get the new data.')
    key, subkey = jax.random.split(key)
    # make sure data on each host is initialized differently
    subkey = jax.random.fold_in(subkey, jax.process_index())
    # create electron state (position and spin)
  ...
  train_schema = ['step','time', 'energy', 'var', 'ewmean', 'ewvar', 'pmove', 'grad_norm', 'learning_rate','momentum','param_norm','precon_grad_norm','update_norm']
